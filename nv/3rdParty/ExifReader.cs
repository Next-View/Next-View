...
namespace Next_View
{
    ...
    public sealed class ExifReader : IDisposable
    {
        ...
        public ExifReader(string fileName)
            : this(new FileStream(fileName, FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite), false, true)
        {
        }
#endif
        ...
        public bool SaveOrient(byte oriByte)
        {
        	if (_oriPos > 0){
        		...
        		_stream.WriteByte(oriByte);   // 1 = normal   0x03 = 180
        		_stream.Close();
         		return true;
        	}
        	else {
