...
namespace Next_View
{
    ...
    public partial class ExifDash : DockContent
    {
        ...
        bool _stop = false;
        bool _dashLeave = true;    // dash width fix
        string _imgDir = "";
        ...
        public bool DashKDown(int kValue, bool ctrl, bool alt)
        {
          //Debug.WriteLine("char: " + kValue);
            switch(kValue)
            {
                case 87:    // ctrl w     close
                    if (ctrl){
                        this.Close();
                    }
                    break;
        ...
        // called by: bw end
        {
            ...
            if (_dateCount > 0){
                ...
                else if (imgSpan.TotalDays > 1 || (sameDay == false     )){
                    _rangeType = 3;      // days
        ...
        void ExifDashHelpRequested(object sender, HelpEventArgs hlpevent)
        {
        ...
        void ExifDashKeyDown(object sender, KeyEventArgs e) 
        {
            bool alt = false;
            ...
            DashKDown(e.KeyValue, ctrl, alt);
            //else Debug.WriteLine("eat up1: " + e.KeyValue);   
        }

        void CmdStartKeyDown(object sender, KeyEventArgs e)
        {
            bool alt = false;
            if (e.Modifiers == Keys.Alt){
                alt = true;
        ...
        void ExifDashLoad(object sender, EventArgs e)
        {
            ...
            Debug.WriteLine("this h: " + this.Height);
            if (listImg.Height < 100) { 
                listImg.Height = 140;
                panel1.Height += 222;
            }
            Debug.WriteLine("list img: " + listImg.Height);
            Debug.WriteLine("this h2: " + this.Height);
        }
        // ------------------------------   events  ----------------------------------------------------------
        ...
        void ListImgDoubleClick(object sender, EventArgs e)
        {
            if (listImg.SelectedItems.Count > 0){
        ...
        void CmdShowClick(object sender, EventArgs e)
        {
            if (_exifImgList.Count > 0){
                string eImg = listImg.Items[0].Text;  // start img, first or selected
                if (listImg.SelectedItems.Count > 0){
